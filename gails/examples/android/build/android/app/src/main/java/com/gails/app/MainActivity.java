...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String WAILS_HOST = "gails.localhost";
    ...
    private void setupWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                ...
                // Handle gails.localhost requests
                if (request.getUrl().getHost() != null &&
                        request.getUrl().getHost().equals(WAILS_HOST)) {

                    // For gails API calls (runtime, capabilities, etc.), we need to pass the full URL
                    // including query string because WebViewAssetLoader.PathHandler strips query params
                    String path = request.getUrl().getPath();
                    if (path != null && path.startsWith("/gails/")) {
