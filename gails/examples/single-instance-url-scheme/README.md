...
Demonstrates (and formerly reproduced) [wailsapp/gails#5089](https://github.com/wailsapp/gails/issues/5089):
...
Requirements: macOS, Go, `gails3` CLI (`task` runner), Xcode command-line
...
tail -f /tmp/gails-single-instance-url.log
```
...
gails3 task trigger URL='gails-single-url://hello?n=1'

# Or directly:
open 'gails-single-url://hello?n=1'
```

### macOS version behaviour differences

**macOS 14 / 15:** `open 'gails-single-url://...'` causes LaunchServices to
...
**macOS 26+ (observed on 26.0/25A354, Apple Silicon):** `open 'gails-single-url://…'`
...
```sh
gails3 task trigger:force URL='gails-single-url://hello?n=1'
```
...
[first]   Args           = [.../single-instance-url-scheme gails-single-url://hello?n=1]
[first]   url-in-args?   = true  (url="gails-single-url://hello?n=1")
```
...
- Running the raw binary (`go run .`) will **not** exercise the bug path.
  ...
  bundle registered with LaunchServices. Use `gails3 task run`.
- If `open 'gails-single-url://…'` launches a different app, the scheme
