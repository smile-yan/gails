# Cross-Platform Testing Guide for Gails v3

This document describes the comprehensive cross-platform testing system for Gails v3 examples, supporting Mac, Linux, and Windows compilation.

## Overview

The testing system ensures all Gails v3 examples build successfully across all supported platforms:
...
The system builds all 43 Gails v3 examples:
...
**Docker Images:**
- `gails-v3-linux-arm64` - Ubuntu 24.04 ARM64 native compilation (built from `test/docker/Dockerfile.linux-arm64`)
- `gails-v3-linux-x86_64` - Ubuntu 24.04 x86_64 native compilation (built from `test/docker/Dockerfile.linux-x86_64`)
- `gails-v3-linux-fixed` - Legacy unified image (deprecated)
...
- **Fix**: Standardized all examples to use `replace github.com/wailsapp/gails/v3 => ../..`
...
```
**Solution**: All examples now use standardized `replace github.com/wailsapp/gails/v3 => ../..`
...
docker build -f Dockerfile.linux -t gails-v3-linux-builder .
docker build -f Dockerfile.linux-simple -t gails-v3-linux-simple .
```
...
- [Gails v3 Documentation](https://gails.io/docs/)
