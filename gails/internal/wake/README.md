...
> not the default runtime. The default `gails3 build / package / sign /
...
Wake is an **experimental alternative build runner** for `gails3`. It reads
...
Gails-aware executor instead of the general-purpose Task runtime. The goal
isn't to replace Task; it's to provide a runner that's purpose-built for
the way Gails projects actually build, with semantics, output and defaults
that match the rest of the gails3 CLI. If you only ever use Wake, your
Taskfiles do not change.

## How Wake differs from Task

Both Wake and the Task runtime are compiled into `gails3` — neither
requires a separate binary install. The differences are about what each
one is *for*.

### 1. Tailored to the Gails build pipeline

Wake knows the shape of a Gails build: the frontend bundle gets embedded
...
### 3. Structured output controlled by gails3

Wake renders through gails3's own **Pulse** reporter: a pre-painted
...
gails3 CLI; nothing about it changes when the underlying task moves.
...
`true`), every gails3 command — `gails3 build`, `gails3 package`,
`gails3 sign`, `gails3 task <name>` — uses the embedded Task runtime
...
WAILS_USE_WAKE=true gails3 build
WAILS_USE_WAKE=true gails3 package
WAILS_USE_WAKE=true gails3 task <some-task-name>
```

What the gate covers:

- `commands.wrapTask` (used by `gails3 build / package / sign`) routes
  through `wake.Execute` only when `useWake()` is true.
- `commands.RunTask` (used by `gails3 task <name>`) does the same —
  `gails3 task` is consistent with the other verbs.
- `gails3 dev` is **not** affected by this flag yet; the dev watcher
...
compiled into `gails3` — there is no external `task` binary to install
...
| Variable             | Effect                                                                            |
| -------------------- | --------------------------------------------------------------------------------- |
| `WAILS_USE_WAKE`     | `true` enables wake-routable gails3 verbs; anything else uses the Task runtime    |
