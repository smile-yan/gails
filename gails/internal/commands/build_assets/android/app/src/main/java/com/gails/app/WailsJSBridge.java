...
import com.gails.app.BuildConfig;
...
public class WailsJSBridge {
     ...
     * Called from JavaScript: gails.invoke(message)
     *
     ...
     * Called from JavaScript: gails.invokeAsync(callbackId, message)
     *
     ...
     * Called from JavaScript: gails.log(level, message)
     *
     ...
     * Called from JavaScript: gails.platform()
     *
     * @return "android"
     */
    @JavascriptInterface
     ...
     * Called from JavaScript: gails.isDebug()
     *
    ...
    private void sendCallback(String callbackId, String result, String error) {
        ...
        if (error != null) {
            js = String.format(
                    "window.gails && window.gails._callback('%s', null, '%s');",
                    escapeJsString(callbackId),
                    escapeJsString(error)
            );
        } else {
            js = String.format(
                    "window.gails && window.gails._callback('%s', %s, null);",
                    escapeJsString(callbackId),
