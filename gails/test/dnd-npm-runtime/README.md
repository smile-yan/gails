...
This test verifies that drag-and-drop functionality works correctly when using the `@wailsio/runtime` npm module instead of the bundled `/gails/runtime.js`.

## Background

There was a bug where the Go backend called `window.gails.Window.HandlePlatformFileDrop()` for native file drops (macOS/Linux), but the npm module only registers the handler at `window._wails.handlePlatformFileDrop`.

The bundled runtime sets `window.gails = Runtime`, so the call worked. But with the npm module, `window.gails` is an empty object.
...
"window.gails.Window.HandlePlatformFileDrop(%s, %d, %d);"
