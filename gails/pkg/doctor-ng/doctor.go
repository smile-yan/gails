...
func (d *Doctor) generateSummary() {
	...
	if len(missing) == 0 && errCount == 0 {
		d.report.Ready = true
		if warnCount > 0 {
			d.report.Summary = "System is ready for Gails development with some warnings"
		} else {
			d.report.Summary = "System is ready for Gails development!"
		}
	} else {
