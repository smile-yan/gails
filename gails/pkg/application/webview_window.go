...
func (w *WebviewWindow) HandleMessage(message string) {
	...
	case strings.HasPrefix(message, "gails:resize:"):
		if !w.IsFullscreen() {
	...
	case message == "gails:runtime:ready":
	...
	case strings.HasPrefix(message, "gails:event:emit:"):
		...
		if !w.options.AllowSimpleEventEmit {
			w.Error("gails:event:emit received but AllowSimpleEventEmit is not set on this window: %v", message)
			return
		}
