...
// /gails/runtime.js can never be imported (fetch fails). This shim
// installs the minimum subset of the runtime that postMessage-based
// custom-event traffic needs:
//
//   * window._gails.dispatchGailsEvent  — the framework calls this when
//     the host emits a custom event to the page.
//   * window.gails.Events.On(name, cb)  — the page subscribes.
//   * window.gails.Events.Emit(name)    — the page fires a bare-name
//     event back to the host. Routed through
//     `window._gails.invoke("gails:event:emit:" + name)` which the
//     framework forwards if the owning window has
//     WebviewWindowOptions.AllowSimpleEventEmit set.
//
// Once the platform layer has injected window._gails.invoke the shim
// fires `gails:runtime:ready` so any pending host-side queued events
// flush. If the modern HTTP runtime later loads it will overwrite
// window.gails.Events with its richer implementation — that's fine,
...
// don't fail on parse.
(function () {
    var w = window._gails = window._gails || {};
    if (window.gails && window.gails.Events) {
        return; // a full runtime is already in scope
    }
    var listeners = Object.create(null);
    w.dispatchGailsEvent = w.dispatchGailsEvent || function (ev) {
    ...
    window.gails.Events = {
        ...
        Emit: function (eventOrName) {
            ...
            w.invoke("gails:event:emit:" + name);
        },
    };
    (function ready() {
        if (typeof w.invoke === "function") {
            w.invoke("gails:runtime:ready");
        } else {
            setTimeout(ready, 30);
        }
    })();
